...
define(['i18n!orion/settings/nls/messages', 'orion/PageUtil', 'orion/objects', 'orion/webui/littlelib', 'orion/i18nUtil'],
		function(messages, PageUtil, objects, lib, i18nUtil) {
	...
	objects.mixin(PluginEntry.prototype, {
							...
							'<div class="plugin-list-item">' + //$NON-NLS-0$
								'<div>' + //$NON-NLS-0$
									...
									'<div class="stretch">' + //$NON-NLS-0$
										'<span class="language-title"></span>' + //$NON-NLS-0$
										'<div class="language-extensions"></div>' + //$NON-NLS-0$
										'<div class="language-description"></div>' + //$NON-NLS-0$
										'<div></div>' + //$NON-NLS-0$
										'<div class="language-status"></div>' + //$NON-NLS-0$
									'</div>' + //$NON-NLS-0$
								'</div>' + //$NON-NLS-0$
							'</div>'+ //$NON-NLS-0$
		...
		createElements: function() {
			...
			this.pluginTitle = lib.$(".language-title", this.node);  //$NON-NLS-0$
			this.pluginExtensions = lib.$(".language-extensions", this.node);  //$NON-NLS-0$ 
			this.pluginStatus = lib.$(".language-status", this.node);  //$NON-NLS-0$ 
			this.pluginDescription = lib.$(".language-description", this.node);  //$NON-NLS-0$ 
		...
		destroy: function() {
			if (this.node) {
				this.node = this.pluginTitle = this.pluginExtensions = this.pluginDescription = this.commandSpan = this.serviceContainer = null;
			}
		},
		show: function() {
			this.createElements();
		...
		postCreate: function() {
			this.pluginTitle.textContent = this.plugin.name || this.plugin.scopeName;
			this.pluginExtensions.textContent = i18nUtil.formatMessage(messages["File types"], this.plugin.fileTypes.join(", "));
			this.pluginDescription.textContent = this.plugin.comment || "";
			if (this.plugin.needsOmiguruma || true) {
				var icon = document.createElement("span"); //$NON-NLS-0$
				icon.className = "core-sprite-warning modelDecorationSprite"; //$NON-NLS-0$
				var content = document.createElement("span"); //$NON-NLS-0$
				content.textContent = messages["Omiguruma warning"]; // Problem during update/starting
				this.pluginStatus.appendChild(icon);
				this.pluginStatus.appendChild(content);
			}

			this.commandSpan.id = this.pluginTitle.textContent;
			this.commandService.renderCommands("commands", this.commandSpan, this.pluginTitle.textContent, this, "tool"); //$NON-NLS-1$ //$NON-NLS-0$
