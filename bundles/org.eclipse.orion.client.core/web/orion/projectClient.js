...
define([
	...
	_normalizeFileName : function(fileName, extension){
		var tmp = fileName;
	...
	saveProjectLaunchConfiguration: function(projectMetadata, configurationName, serviceId, params, url, manageUrl, path, deployType, additionalConfiguration){
		...
		var configurationFile = this._normalizeFileName(configurationName, ".launch"); //$NON-NLS-0$
		...
		this.getLaunchConfigurationsDir(projectMetadata, true).then(function(launchConfDir){
			if(launchConfDir.Children){
				...
				this._updateOrCreate(launchConfDir, configurationFile, launchConfigurationContents).then(function(result){
					...
					if(additionalConfiguration){
						...
						var sharedConfigurationFile = this._normalizeFileName(configurationName, extension);
						this._updateOrCreate(launchConfDir, sharedConfigurationFile, additionalContents).then(function(){
