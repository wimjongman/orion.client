       ...
       _getFile : function _getFile(name, options) {
           ...
           if(file && file.indexOf(dotext) > -1) {
               return this.fileclient.loadWorkspace().then(function(workspace) {
                   ...
                   files = [that._newFileObj(name, '/file/'+workspace.Id+'/org.eclipse.orion.client/'+file, that._trimName(file), icon, type, that.fileclient)];
           ...
           return this.fileclient.search(
                {
                    'resource': that.fileclient.fileServiceRootURL(),
                    'keyword': searchname,
                    'sort': 'Name asc',
                    'nameSearch': true,
                    'fileType': ext,
