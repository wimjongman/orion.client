        ...
        "no-shadow-global": {
            ...
            rule: function(context) {
                ...
                function checkShadow(node) {
                    if(context.env) {
                        var envs = Object.keys(context.env);
                        if(envs.length < 1) {
                        ...
                        switch(node.type) {
                            case 'VariableDeclarator': {
                                if(context.env[Finder.findESLintEnvForMember(node.id.name)]) {
                            ...
                            case 'FunctionDeclaration': {
                                node.params.forEach(function(param) {
                                    if(param.type === 'Identifier' && context.env[Finder.findESLintEnvForMember(param.name)]) {
